...
class CustomSlider extends FlxSpriteGroup
{
	...
	private function CreateSlider():Void
	{
		...
		var barPos:FlxPoint = new FlxPoint(0, ((barHeight - _barThickness) / 2));
		var handPos:FlxPoint = new FlxPoint(0, ((barHeight - _handleThickness) / 2));
		
		_bounds = new FlxRect(x, y, _barWidth + _handleWidth, barHeight);
